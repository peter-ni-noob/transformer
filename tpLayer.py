...
class WPColumnParallelLinear(torch.nn.Module):
    """Linear layer with column parallelism.
    ...
    def __init__(
        ...
        super(WPColumnParallelLinear, self).__init__()
...
class WPRowParallelLinear(torch.nn.Module):
    """Linear layer with row parallelism.
    ...
    def __init__(
        ...
        super(WPRowParallelLinear, self).__init__()
        # Keep input parameters
        self.input_size = input_size
        self.output_size = output_size
        self.input_is_parallel = input_is_parallel
...
class WPParallelAttention(nn.Module):
    """
    Megatron ParallelAttention
    """
    def __init__(self,config:TransformerConfig,
        ...
        bias: bool = True,
        batch_first: bool =False):
        super(WPParallelAttention,self).__init__()
        self.batch_first=batch_first
        self.layer_number=max(1,layer_number)
        self.config=config
        kv_channels=embed_dim/num_heads
        ...
        if attention_type == AttnType.self_attn:
           self.mixed_x_layer=WPColumnParallelLinear(config,hidden_size, int(query_projection_size + 2 * kv_projection_size),bias,False)
        else:
            assert False,"not implemented!"
        
        self.outmha=WPRowParallelLinear(config,int(query_projection_size),hidden_size,bias,True)
        if attention_mask:
            self.register_buffer("attention_mask",nn.Transformer.generate_square_subsequent_mask(config.seq_len,dtype=torch.float32))
        else:
            self.register_buffer("attention_mask",None)
...
class WPParallelMLP(nn.Module):
    def __init__(self,config:TransformerConfig,layer_number:int=-1):
        ...
        self.gelu    = nn.GELU(approximate='tanh')
        self.c_proj  = WPRowParallelLinear(config,config.intermidiate_size, config.n_embd,bias=True,input_is_parallel=True)
        self.c_proj.name="c_proj"
        self.c_proj.opname="linear"
    ...
    def forward(self,x):
        ...
        with torch.no_grad():
            with torch.cuda.stream(gobalVar.COPYSTREAM):
                x_copy=get_global_memory_buffer().get_tensor(x.shape,x.dtype,"x")
                x_copy.copy_(x,non_blocking=True)
        ...
        for now in range(gobalVar.TPWORLD_SIZE):
            ...
            for name,param in self.named_parameters():
                if name.find("ln")!=-1:
                    continue
                gpu_tensor=get_global_memory_buffer().try_get_tensor(param.shape,param.dtype,"rank-"+str(now)+"-"+"mlp."+name)
                param.data.copy_(gpu_tensor)
            with torch.no_grad():
                x1=self.c_fc(x_copy)
                x1=self.gelu(x1)
                x1,bias1=self.c_proj(x1)
                x+=x1
        for name,param in self.named_parameters():
            if name.find("ln")!=-1:
                continue
            gpu_tensor=get_global_memory_buffer().try_get_tensor(param.shape,param.dtype,"rank-"+str(self.rank)+"-"+"mlp."+name)
            with torch.cuda.stream(gobalVar.COPYSTREAM):
                param.data.copy_(gpu_tensor,non_blocking=True)        
        x=x+bias
        # gobalVar.COPYSTREAM.synchronize()
