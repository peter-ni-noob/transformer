...
tmp_dir = mpath+"nexusnet/build/Pdata/"#改保存路径
...
def save_npy(basename: str, x: Union[torch.Tensor, np.ndarray]):
    global isdebug
    global iteration
    if(isdebug==False or iteration!=2):
        return
...
def save_npy_b(basename: str, x: Union[torch.Tensor, np.ndarray]):
    global isdebug
    if(isdebug_back==False or iteration!=3):
        return
    if isinstance(x, torch.Tensor):
        x = x.contiguous().cpu().detach().numpy().astype(np.float32)
    assert isinstance(x, np.ndarray)
...
def hook_forward_function(module, module_input, module_output):
    ...
    pass
#grad 按拓扑排序
def hook_backward_function_g(module):
...
#grad 按拓扑排序
def hook_backward_function(module):

    if(hasattr(module,"name")):
        if(module.opname=="layernorm"):
            save_npy_b(genSavePath_b(module.name+"_gamma.npy"),module.weight)
            save_npy_b(genSavePath_b(module.name+"_beta.npy"),module.bias)
        elif(module.opname=="mha"):
            # 1/0
            # save_npy_b(genSavePath_b("back_"+module.name+"_output_x.npy"),module_input_grad[0]+module_input_grad[1]+module_input_grad[2])
            # save_npy_b(genSavePath_b("back_"+module.name+"_input_g.npy"),module_output_grad[0])
            
            save_npy_b(genSavePath_b(module.name+"in_proj_bias.npy"),module.in_proj_bias)
            save_npy_b(genSavePath_b(module.name+"in_proj_weight.npy"),module.in_proj_weight)
            save_npy_b(genSavePath_b(module.name+"out_proj_bias.npy"),module.out_proj.bias)
            save_npy_b(genSavePath_b(module.name+"out_proj_weight.npy"),module.out_proj.weight)
        elif(module.opname=="linear"):
            save_npy_b(genSavePath_b(module.name+"_weight.npy"),module.weight)
            save_npy_b(genSavePath_b(module.name+"_bias.npy"),module.bias)
        # embedding 从hook得不到grad
        elif(module.opname=="embedding"):
            save_npy_b(genSavePath_b(module.name+"_embedding.npy"),module.weight)
...
def save_b_g(model):
    ...
    for i in model.modules():
       if(hasattr(i,"name")):
           hook_backward_function_g(i)

def save_b_w(model):
    grads=collections.OrderedDict()
    for i in model.modules():
       if(hasattr(i,"name")):
           hook_backward_function(i)
...
def main():
    ...
    state_dict0=torch.load("/data/Pweight/gpt2xlHugginFace_pweight.pth") # /data/Pweight/0_gpt2xl.pth  gpt2xlHugginFace_pweight.pth
    ...
    model.apply(model.init_grouped_weight)#自定义初始化权重
    torch.save({"model_state_dict":save_dic_toCPU(model.state_dict())},"./gpt2xl_weight0.pt")
    ...
    for step,(data,label) in enumerate(gptdataloader):
        ...
        if (step+1)%accumulation_steps ==0:
            ...
            #     if p.grad is not None:
            #         aa.append(p.grad)
            #         # if p.shape[0]==50257 and p.shape[1]==1600:
            #         #     x = p.grad.contiguous().cpu().detach().numpy().astype(np.float32)
            #         #     np.save("grade0",x)
            # print(len(aa))
            ...
            #             save_npy_b(genSavePath_b(newName),value.grad)
            #             continue
            ...
            # if iteration==3:
            #     save_npy_raw("/root/workspace/nexus_gpt2/nexusnet/build/Pdata0/embed_w.npy",model.transformer.wpe.weight)
                # save_npy_raw("/root/workspace/nexus_gpt2/nexusnet/build/Pdata0/embed_x.npy",model.pos_emb_x)
            # for name,value in model.named_parameters():
            #     print(name)
            # save_b_g(model)
            a=torch.nn.utils.clip_grad_norm_(model.parameters(),1.0,foreach=False)
            # save_npy_raw("/root/workspace/nexus_gpt2/nexusnet/build/tmp/p0",a)
