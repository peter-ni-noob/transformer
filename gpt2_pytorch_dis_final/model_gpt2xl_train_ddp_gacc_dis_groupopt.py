...
def main():
    ...
    state_dict0=torch.load("/data/Pweight/gpt2xl_weight0.pt") # /data/Pweight/0_gpt2xl.pth  gpt2xlHugginFace_pweight.pth
    ...
    optimizer = torch.optim.AdamW([{"params":mlpWeight,"lr":Opt_config.init_lr,"weight_decay":Opt_config.wd},
                                   {"params":otherWeight}],lr=1e-9,weight_decay=0.0,foreach=False)
    ...
    for step,(data,label) in enumerate(gptdataloader):
        ...
        if (step+1)%accumulation_steps ==0:
            ...
            #对其他参数进行梯度裁剪
            torch.nn.utils.clip_grad_norm_(otherWeight,1.0,foreach=False)
            ...
            if(real_step==389 and ddp_rank==0):
                state={
                    "step":real_step,
                ...
                torch.save(state,"/data/Pweight/P%d_gpt2xl.pth"%real_step)
                pass
            real_step+=1
