...
x=torch.randn([4,256,1024],device=device,dtype=torch.float32,requires_grad=True)
save_npy("x.npy",x)
y=torch.randn([4,1024,256],device=device,dtype=torch.float32,requires_grad=True)
save_npy("y.npy",y)

# f=nn.GELU("tanh").to(device)
t_begin=time.time()
for i in range(200*(12*2)*2):
    z=x@y
    z.mean().backward()
t_end=time.time()
